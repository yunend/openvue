...
| `doc` `docx` `xls` `xlsx` `ppt` `pptx` | Office Viewer | Word、Excel、PowerPoint 文档预览（微软在线查看器，⚠️ 需 HTTPS 公网） |

> **⚠️ Office Viewer（需公网）**：`doc`、`docx`、`xls`、`xlsx`、`ppt`、`pptx` 等通过 `https://view.officeapps.live.com` 微软在线查看器渲染，要求文件 URL 必须满足：
